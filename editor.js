class Editor {
  ...
  redraw() {
    ...
    // Create a temporary canvas for the complete image with all elements except blur boxes
    ...
    // First draw all non-blur elements to the temp canvas
    this.elements.forEach((element, index) => {
      if (element.type !== 'blur') {
        if (element.type === 'box') {
          this.drawBox(element, tempCtx);
        } else if (element.type === 'text') {
          tempCtx.font = element.font;
          tempCtx.textBaseline = 'alphabetic';
          if (element.borderSize > 0) {
            tempCtx.strokeStyle = element.borderColor;
            tempCtx.lineWidth = element.borderSize;
            tempCtx.strokeText(element.content, element.x, element.y);
          }
          tempCtx.fillStyle = element.color;
          tempCtx.fillText(element.content, element.x, element.y);
        } else if (element.type === 'image') {
          tempCtx.drawImage(element.content, element.x, element.y, element.width, element.height);
        }
      }
    });

    // Copy the base content to the main canvas
    this.ctx.drawImage(tempCanvas, 0, 0);

    // Now apply blur boxes
    this.elements.forEach((element, index) => {
      if (element.type === 'blur') {
        // Create a temporary canvas for each blur region
        const blurCanvas = document.createElement('canvas');
        blurCanvas.width = element.width;
        blurCanvas.height = element.height;
        const blurCtx = blurCanvas.getContext('2d');

        // Copy the region to be blurred
        blurCtx.drawImage(
          tempCanvas,
